...
async function proxy(request: Request, method: "GET" | "PUT") {
  ...
  for (const base of urls) {
    try {
      const response = await fetch(`${base}/community-channels`, {
        method,
...
export async function GET(request: Request) {
  try {
    ...
    console.error("Community channels GET proxy error:", error);
    return NextResponse.json({ detail: "Error en el proxy de canales de comunidad" }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    console.error("Community channels PUT proxy error:", error);
    return NextResponse.json({ detail: "Error en el proxy de canales de comunidad" }, { status: 500 });
  }
}
